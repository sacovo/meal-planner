...
class CampCollaborationTest(TestCase):
    def setUp(self):
        ...
        self.collaborator = User.objects.create_user(username="collab", password="password")
    ...
    def test_collaborator_can_remove_self(self):
        self.client.force_login(self.collaborator)
        response = self.client.delete(f"/api/meals/camps/{self.camp.id}/collaborators/collab")
        self.assertEqual(response.status_code, 200)
