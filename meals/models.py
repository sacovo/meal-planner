...
class Camp(models.Model):
    ...
    collaborators = models.ManyToManyField(
        User, related_name="collaboration_camps", blank=True
    )
...
class IngredientUnitConversion(models.Model):
    ingredient = models.ForeignKey(
        Ingredient, on_delete=models.CASCADE, related_name="conversions"
    )
    unit_name = models.CharField(
        max_length=50, help_text="e.g. tbsp, tsp, piece, kilogram"
    )
...
class Recipe(models.Model):
    ...
    collaborators = models.ManyToManyField(
        User, related_name="collaboration_recipes", blank=True
    )
    ...
    preferences = models.ManyToManyField(
        DietaryPreference, related_name="recipes", blank=True
    )
    default_portions = models.PositiveIntegerField(
        default=1, help_text="How many people this recipe serves as written"
    )
    is_importing = models.BooleanField(
        default=False,
...
class CampMeal(models.Model):
    ...
    override_people_count = models.PositiveIntegerField(
        null=True, blank=True, help_text="Overrides camp's default count"
    )
    serves_preference = models.ForeignKey(
        DietaryPreference,
    ...
    leftovers_noted = models.TextField(
    ...
    def people_count(self):
        return (
            self.override_people_count
            if self.override_people_count is not None
            else self.camp.default_people_count
        )
...
class GeneralCampItem(models.Model):
    camp = models.ForeignKey(
        Camp, on_delete=models.CASCADE, related_name="general_items"
    )
    name = models.CharField(max_length=255)
...
class ShoppingList(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    camp = models.ForeignKey(
        Camp, on_delete=models.CASCADE, related_name="shopping_lists"
    )
...
class ShoppingListItem(models.Model):
    ...
    source_meals_text = models.JSONField(
        default=list,
        blank=True,
