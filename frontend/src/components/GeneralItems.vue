...
<template>
  <div class="card general-items">
    ...
    <div class="flex gap-2 input-group">
      ...
      />
      <button class="btn btn-primary add-btn" @click="handleAdd">
        {{ t("btn.add") }}
      </button>
    </div>
    ...
    <ul class="items-list">
      <li v-for="item in items" :key="item.id!" class="item-row">
        ...
        <button
          class="text-mute delete-btn"
          @click="$emit('delete', String(item.id))"
          :title="t('btn.delete')"
        >
          ✕
        </button>
      </li>
      <li v-if="items.length === 0" class="text-mute empty-msg">
        {{ t("misc.no_data") }}
      </li>
    </ul>
  </div>
