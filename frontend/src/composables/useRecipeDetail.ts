...
export function useRecipeDetail(recipeId: string) {
  ...
  const isOwner = computed(
    () => recipe.value && currentUser.value === recipe.value.owner_username,
  );
  const isCollaborator = computed(
    () =>
      recipe.value &&
      recipe.value.collaborators?.includes(currentUser.value || ""),
  );
  ...
  async function fetchRecipe() {
    const { data: r } = await mealsApiRecipesGetRecipe({
    ...
    if (r) {
      ...
      editRecipeData.value = {
        ...
        preference_ids: r.preferences
          ? r.preferences.map((p: any) => p.id)
          : [],
        tags: r.tags || [],
      };
  ...
  async function fetchIngredients() {
    const { data } = await mealsApiRecipesListRecipeIngredients({
      path: { recipe_id: recipeId },
    });
  ...
  watch(ingredientQuery, async (newQuery) => {
    ...
    const { data } = await mealsApiIngredientsListIngredients({
      query: { q: newQuery },
    });
    if (data) {
