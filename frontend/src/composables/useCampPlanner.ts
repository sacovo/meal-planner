...
export function useCampPlanner(campId: string) {
  ...
  const currentUser = ref<{ is_logged_in?: boolean; username?: string | null }>(
    { is_logged_in: false },
  );
  ...
  const latestShoppingListId = computed(
    () => shoppingLists.value[0]?.id as string | undefined,
  );
  ...
  const campDays = computed(() => {
    if (!camp.value || !camp.value.start_date || !camp.value.end_date)
      return [];
    const arr: string[] = [];
  ...
  async function fetchData() {
    ...
    const { data: mealsData } = await mealsApiMealsListCampMeals({
      path: { camp_id: campId },
    });
    if (mealsData) {
  ...
  function toggleDay(day: string) {
    ...
    if (isDaySelected(day)) {
      selectedMeals.value = selectedMeals.value.filter(
        (id) => !dayMealIds.includes(id),
      );
    } else {
      const existing = selectedMeals.value.filter(
        (id) => !dayMealIds.includes(id),
      );
  ...
  async function fetchShoppingLists() {
    ...
    try {
      const { data } = await mealsApiShoppingListCampShoppingLists({
        path: { camp_id: campId },
      });
  ...
  async function fetchGeneralItems() {
    const { data } = await mealsApiCampsListCampGeneralItems({
      path: { camp_id: campId },
    });
  ...
  async function deleteGeneralItem(id: string) {
    await mealsApiCampsDeleteCampGeneralItem({
      path: { camp_id: campId, item_id: id },
    });
    fetchGeneralItems();
  }
  ...
  async function moveGeneralItemsToShoppingList() {
    ...
    if (!confirm(t("confirm.move_general_items_to_shopping_list"))) return;
  ...
  // ---- Camp editing ----
  async function saveEditCamp(data: {
    name: string;
  ...
  async function removeCollaborator(username: string) {
    ...
    try {
      await mealsApiCampsRemoveCollaborator({
        path: { camp_id: campId, username },
      });
      if (camp.value && camp.value.collaborators) {
        camp.value.collaborators = camp.value.collaborators.filter(
          (u: string) => u !== username,
        );
      }
