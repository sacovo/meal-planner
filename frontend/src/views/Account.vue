...
function getCsrfToken() {
  return getCookie('csrftoken') || (document.querySelector('[name=csrfmiddlewaretoken]') as HTMLInputElement)?.value || ''
}
...
async function updateProfile() {
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json()
      account.value = data
      profileMsg.value = { type: 'success', text: t('account.profile_updated') }
    } else {
      const err = await res.json()
      profileMsg.value = { type: 'danger', text: err.detail || t('misc.error') }
    }
  } catch (err) {
...
async function changePassword() {
  ...
  try {
    ...
    if (res.ok) {
      passwordMsg.value = { type: 'success', text: t('account.password_changed') }
      old_password.value = ''
      new_password.value = ''
      confirm_password.value = ''
    } else {
      const err = await res.json()
      passwordMsg.value = { type: 'danger', text: err.error || err.detail || t('misc.error') }
    }
  } catch (err) {
