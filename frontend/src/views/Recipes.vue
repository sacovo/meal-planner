...
const { recipes, isLoading, totalCount, hasMore, fetchRecipes, loadMore } =
  useRecipeList({
    searchQuery,
    selectedTags,
...
async function handleImportRecipe() {
  ...
  try {
    const { data } = await mealsApiRecipesImportRecipe({
    ...
    <div class="flex gap-4 items-end flex-wrap mb-8">
      ...
      <div class="filter-field filter-field-narrow">
        ...
        <select v-model="selectedPreferenceId" class="input filter-select">
          ...
          <option v-for="p in allPreferences" :key="p.id!" :value="p.id">
            {{ p.name }}
          </option>
        </select>
      </div>
    ...
    <div class="flex-col gap-8 mt-8">
      <div class="grid-cards">
        ...
        >
          <h3>{{ recipe.name }}</h3>
          <p class="text-mute">
            {{ recipe.default_portions }} {{ t("recipe.portions") }}
          </p>
          <div
            class="flex gap-1 flex-wrap mt-2"
      ...
      <div
        ...
        class="text-mute text-center py-8"
      >
        {{ t("recipe.no_results") }}
      </div>
      ...
      <div
        ...
        class="text-center text-mute text-sm mt-8"
      >
        ...
        <div class="flex-col gap-4 mt-4">
          ...
          <div>
            ...
            <input
              class="input"
              type="number"
              v-model="newRecipe.default_portions"
            />
          </div>
          <div>
          ...
          <div class="flex gap-2 justify-end mt-4">
            ...
            <button class="btn btn-primary" @click="handleCreateRecipe">
              {{ t("btn.create") }}
            </button>
          </div>
        </div>
