...
async function exportExcel() {
  const res = await mealsApiExportInventoryExcel({
    path: { camp_id: campId },
    parseAs: 'blob'
  })
  if (res.data) {
    ...
    a.href = url
    a.download = `inventory_${camp.value?.name || 'camp'}.xlsx`
...
<template>
  <div v-if="camp" class="flex-col gap-6">
    <div class="flex justify-between items-center">
      <div>
        <RouterLink :to="`/camps/${campId}`" class="badge" style="margin-bottom: 0.5rem">{{ t('btn.back') }}
        </RouterLink>
        <h2 style="margin: 0;">{{ t('inventory.title') }}: {{ camp.name }}</h2>
    ...
    <div v-if="successMessage" class="card"
      style="background: var(--color-success); color: white; margin-bottom: 1rem; border: none; font-weight: bold; animation: slideIn 0.3s ease;">
      {{ successMessage }}
    </div>
    ...
    <div v-else class="card" style="padding: 0;">
      <div class="table-container" style="overflow-x: auto;">
        <table style="width: 100%; border-collapse: collapse; min-width: 800px;">
          <thead>
            <tr
              style="text-align: left; font-size: 0.85rem; color: var(--color-text-mute); border-bottom: 2px solid var(--color-border); background: var(--color-bg-mute);">
              <th style="padding: 1rem 0.75rem;">{{ t('inventory.ingredient') }}</th>
              <!-- In Stock -->
              <th style="padding: 1rem 0.25rem; text-align: right;">{{ t('inventory.in_stock') }}</th>
              <th style="padding: 1rem 0.25rem; text-align: left;"></th>
              <!-- Needed -->
              <th style="padding: 1rem 0.25rem; text-align: right;">{{ t('inventory.still_needed') }}</th>
              <th style="padding: 1rem 0.25rem; text-align: left;"></th>
              <!-- Balance -->
              <th style="padding: 1rem 0.25rem; text-align: right;">{{ t('inventory.balance') }}</th>
              <th style="padding: 1rem 0.75rem; text-align: left;"></th>
              <th style="padding: 1rem 0.75rem; text-align: center;">{{ t('inventory.add_to_list') }}</th>
            </tr>
          </thead>
          <tbody>
            <template v-for="cat in categories" :key="cat">
              ...
              <tr style="background: var(--color-bg-base);">
                <td colspan="8" style="padding: 0.75rem; border-bottom: 1px solid var(--color-border);">
                  <h3
                    style="margin: 0; text-transform: uppercase; font-size: 0.85rem; letter-spacing: 1.5px; color: var(--color-primary);">
                    {{ cat }}
                  </h3>
                </td>
              </tr>
              ...
              <tr v-for="item in getItemsForCategory(cat)" :key="item.ingredient_id as string"
                style="border-bottom: 1px solid var(--color-bg-mute);" class="inventory-row">
                <!-- Name -->
                ...
                <td
                  style="padding: 0.75rem 0.25rem; text-align: left; width: 60px; color: var(--color-text-mute); font-size: 0.85rem;">
                  {{ item.unit }}
                </td>
                ...
                <!-- Balance -->
                <td style="padding: 0.75rem 0.25rem; text-align: right; font-weight: bold; width: 100px;"
                  :style="{ color: getBalanceColor(item.balance) }">
                  <span v-if="item.balance > 0">+</span>{{ parseFloat(item.balance.toFixed(2)) }}
                </td>
                <td style="padding: 0.75rem 0.75rem; text-align: left; font-weight: bold;"
                  :style="{ color: getBalanceColor(item.balance) }">
                  {{ item.unit }}
                </td>
                <td style="padding: 0.75rem; text-align: center;">
                  <button class="btn btn-secondary btn-tiny" title="Add to latest shopping list"
    ...
    <div class="card" style="background: var(--color-bg-mute); border: 1px dashed var(--color-border);">
      ...
      <ul style="font-size: 0.85rem; padding-left: 1.2rem; line-height: 1.6; margin: 0;">
        ...
        <li><strong>Still Needed:</strong> Total requirement calculated from all planned meals that are NOT yet marked
          as
          "Cooked".</li>
