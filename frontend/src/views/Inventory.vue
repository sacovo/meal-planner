...
async function exportExcel() {
  ...
  if (res.data) {
    downloadBlob(
      ...
      `inventory_${camp.value?.name || "camp"}.xlsx`,
    );
  }
}
...
const categories = computed(() => {
  ...
  return Array.from(cats)
    .sort()
    .map((c) => {
      return { label: t(`category.${c.toLowerCase()}`), value: c };
    });
});
  ...
  <div v-if="camp" class="flex-col gap-4">
    <div class="page-header card">
      <div class="header-top">
        <RouterLink :to="`/camps/${campId}`" class="badge mb-2">{{
          t("btn.back")
        }}</RouterLink>
    ...
    <div v-else>
      ...
      >
        <div class="drawer-header">
          <h3 class="mb-0">{{ t("inventory.quick_nav") }}</h3>
        ...
        <nav>
          <ul class="quick-nav-list">
            <li v-for="cat in categories" :key="`drawer-${cat.value}`">
              <a
                class="quick-nav-link"
                :href="`#${getCategoryAnchor(cat.value)}`"
                @click="closeNav"
              >
                <span>{{ cat.label }}</span>
                <span class="text-mute">{{
                  getItemsForCategory(cat.value).length
                }}</span>
              </a>
            </li>
          </ul>
      ...
      <div class="inventory-layout">
              ...
              <tbody>
                <template v-for="cat in categories" :key="cat.value">
                  <tr class="category-row">
                    <td colspan="8">
                      <h3
                        :id="getCategoryAnchor(cat.value)"
                  ...
                  <tr
                    v-for="item in getItemsForCategory(cat.value)"
                    :key="item.ingredient_id as string"
                  >
                    <td>
                    ...
                    <td class="col-action">
                      <button
                        ...
                        title="Add to latest shopping list"
        ...
        >
          <h3 class="mb-2">{{ t("inventory.quick_nav") }}</h3>
          <nav>
            <ul class="quick-nav-list">
              <li v-for="cat in categories" :key="`desktop-${cat.value}`">
                <a
                  class="quick-nav-link"
                  :href="`#${getCategoryAnchor(cat.value)}`"
                >
    ...
    <div v-if="showAddModal" class="modal-backdrop">
      <div class="modal">
        ...
        <p v-if="selectedItem" class="mb-4">
          {{ t("inventory.add_to_list_question") }} ({{
