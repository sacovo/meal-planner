...
async function handleCreateCamp() {
  if (
    !newCamp.value.name ||
    !newCamp.value.start_date ||
    !newCamp.value.end_date
  )
    return;
    ...
    <div class="grid-cards">
      <div v-for="camp in camps" :key="camp.id as string" class="card">
        ...
        <p class="text-mute mb-4">
          {{ camp.start_date }} {{ t("dashboard.to") }} {{ camp.end_date }} •
          {{ camp.default_people_count }} {{ t("misc.people") }}
        </p>
        <button
          class="btn btn-secondary"
          @click="router.push(`/camps/${camp.id}`)"
        >
          {{ t("dashboard.open_planner") }}
        </button>
      </div>
    </div>
        ...
        <div class="flex-col gap-4 mt-4">
          ...
          <div>
            ...
            <input
              class="input"
              type="number"
              v-model="newCamp.default_people_count"
            />
          </div>
          ...
          <div class="flex gap-2 justify-end mt-4">
            ...
            <button class="btn btn-primary" @click="handleCreateCamp">
              {{ t("btn.create") }}
            </button>
          </div>
        </div>
