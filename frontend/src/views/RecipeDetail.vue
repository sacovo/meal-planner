...
<template>
  <div v-if="recipe" class="recipe-detail">
    ...
    <div class="flex items-center gap-4 mb-8" :class="{ 'blur-bg': recipe.is_importing }">
        ...
        <div v-else class="flex-col gap-4">
          <div>
            ...
            <div class="flex gap-2 flex-wrap mt-2">
              <label v-for="pref in allPreferences" :key="pref.id || 0" class="pref-checkbox">
      ...
      <div class="card">
        ...
        <div v-if="canEdit" class="ingredient-form">
          <div class="flex-1" style="position: relative;">
            ...
            <ul v-if="showSuggestions" class="dropdown">
              <li v-for="res in searchResults" :key="res.id as string" @click="selectSuggestion(res)"
                @mousedown.prevent>
                {{ res.name }} <span class="text-mute text-xs">(Base: {{ res.base_unit }})</span>
              </li>
            </ul>
          </div>
          ...
          <div class="ingredient-unit-col">
            ...
            <ul v-if="showUnitSuggestions && unitSearchResults.length" class="dropdown dropdown-full">
          ...
          <button class="btn btn-primary ingredient-add-btn" @click="handleAddIngredient">{{
            t('btn.add') }}</button>
        </div>
        ...
        <ul class="list-reset flex-col gap-2">
          <li v-for="ing in ingredients" :key="String(ing.id)" class="list-item flex justify-between items-center">
            <div class="flex-1">
              <strong>{{ ing.ingredient.name }}</strong>
            </div>

            <!-- View Mode -->
            <div v-if="editingIngId !== String(ing.id)" class="flex items-center gap-4 justify-end flex-1">
              <span class="text-mute ingredient-amount">{{ ing.amount }} {{ ing.unit }}</span>
              <div v-if="canEdit" class="flex gap-2">
                <button class="btn btn-secondary btn-sm" @click="startEditIng(ing)">✎</button>
                <button class="btn btn-ghost btn-sm text-danger" @click="removeIngredient(ing)">✕</button>
              </div>
            </div>

            <!-- Edit Mode -->
            <div v-else class="flex items-center gap-2 justify-end flex-1">
              <input type="number" step="0.01" class="input input-inline ingredient-edit-amount"
                v-model="editIngAmount" />
              <input class="input input-inline ingredient-edit-unit" v-model="editIngUnit" />
              <button class="btn btn-primary btn-sm" @click="saveEditIng(ing)">✔</button>
              <button class="btn btn-secondary btn-sm" @click="editingIngId = null">✕</button>
            </div>
          </li>
    ...
    <div v-if="showCollaboratorModal" class="modal-backdrop">
      <div class="modal">
        <div class="flex justify-between items-center mb-6">
          ...
          <button class="btn-ghost" @click="showCollaboratorModal = false">✕</button>
        </div>

        <p class="text-mute text-sm mb-4">Collaborators can edit the recipe but
        ...
        <ul class="list-reset flex-col gap-2">
          ...
          <li v-for="user in recipe.collaborators" :key="user" class="list-item flex justify-between items-center">
            <span>{{ user }}</span>
            <button class="btn btn-ghost text-danger btn-sm" @click="removeCollaborator(user)">Remove</button>
          </li>
        </ul>
      </div>
    </div>
