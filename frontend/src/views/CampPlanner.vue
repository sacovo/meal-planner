  ...
  <div v-if="camp" class="flex-col gap-4">
    <div class="page-header">
      <div class="flex items-center gap-4">
        ...
        <button class="btn btn-ghost" v-if="camp" @click="editingCamp = true">⚙️ {{ t('edit') }}</button>
      </div>

      <div class="flex gap-2">
        <button class="btn btn-secondary no-print" @click="showCollaboratorsModal = true">👥 {{ t('collaborators.title') }}</button>
        <button class="btn btn-secondary no-print" @click="router.push(`/camps/${campId}/inventory`)">📦 {{ t('inventory.title') }}</button>
        <button class="btn btn-secondary no-print" @click="exportMatrixPDF">📋 {{ t('export') }}</button>
      </div>
    </div>

    <!-- Planner Matrix Area -->
    <div class="planner-layout" :class="{ 'sidebar-collapsed': isSidebarCollapsed }">
      ...
      <!-- Matrix Canvas -->
      <PlannerMatrix :camp="camp" :camp-days="campDays" :meal-types-config="MEAL_TYPES" :meals-grid="mealsGrid"
        v-model:selected-meals="selectedMeals" :recipe-names="recipeNames" @drop="onDrop" @edit-meal="openEditMeal"
        @toggle-done="toggleMealDone" @remove-meal="handleRemoveMeal" @switch-day="switchToDayDetail"
        @toggle-day="toggleDay" />
    </div>

    <ShoppingListManager :selected-meals-count="selectedMeals.length" v-model:showShoppingLists="showShoppingLists"
      :shopping-lists="shoppingLists" :loading="loadingShoppingLists" @generate="handleGenerateShoppingList"
    ...
    <div class="grid bottom-grid">
      ...
      <GeneralItems :items="generalItems" :can-move="!!latestShoppingListId" :is-moving="isMovingGeneralItems"
        @add="addGeneralItem" @delete="deleteGeneralItem" @move="moveGeneralItemsToShoppingList" />
    </div>
    ...
    <CollaboratorsModal v-if="camp" v-model:show="showCollaboratorsModal" :collaborators="camp.collaborators || []"
      ...
      @invite="inviteCollaborator" @remove="handleRemoveCollaborator" />
