...
vi.mock("../client", () => ({
  ...
  mealsApiRecipesListRecipes: vi.fn(() =>
  ...
  mealsApiShoppingListCampShoppingLists: vi.fn(() =>
    Promise.resolve({ data: [] }),
  ),
  mealsApiIngredientsListTags: vi.fn(() => Promise.resolve({ data: [] })),
  coreApiAccount: vi.fn(() =>
...
describe("CampPlanner.vue", () => {
  ...
  it("opens edit modal when edit button is clicked", async () => {
    ...
    const editBtn = wrapper
      .findAll("button")
      .find((b) => b.text().toLowerCase().includes("edit"));
